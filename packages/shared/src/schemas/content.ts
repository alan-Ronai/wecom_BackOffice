...
export const StepExtrasSchema = z.object({
  signals: z
    .array(
      z.object({ label: z.string(), tone: z.enum(['red', 'blue']).optional(), items: z.array(z.string()) }),
    )
    .optional(),
  pillars: z
    .array(
      z.object({
        label: z.string(),
        text: z.string(),
        tone: z.enum(['ok', 'warn', 'red', 'gray']).optional(),
      }),
    )
    .optional(),
  stages: z
    .array(
      z.object({ label: z.string(), script: z.string().optional(), actions: z.array(z.string()).optional() }),
    )
    .optional(),
...
export const VersionSchema = z.object({
  ...
  authorName: z.string(),
  createdAt: IsoDateSchema,
  suggestionId: IdSchema.nullable().optional(),
});
...
export const DocumentLinkSchema = z.object({
  fromDocumentId: IdSchema,
  fromStepKey: z.string().nullable(),
  toDocumentId: IdSchema.nullable(),
  toBlockId: IdSchema.nullable(),
  toFieldName: z.string().nullable(),
  toSourceId: IdSchema.nullable(),
  type: LinkTypeSchema,
