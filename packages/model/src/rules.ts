      ...
      if (d.kind === 'removed' && linked.length) {
        for (const s of linked)
          out.push({
            anchor,
            type: 'deprecate-step',
            title: 'הוצאה משימוש: ' + s.stepTitle,
            targetDocumentId: s.documentId,
      ...
      if (d.kind === 'added' || !linked.length) {
        ...
        out.push({
          ...
          payload: {
            type: 'new-card',
            title,
            description: after.slice(0, 120),
            category: /חו"ל|נדידה/.test(after)
              ? 'intl'
              : /חיוב|חשבונית/.test(after)
                ? 'billing'
                : /שימור|נטישה/.test(after)
                  ? 'ops'
                  : 'tech',
            wave: 2,
            priority: 'm',
        ...
        if (s.blockId) {
          ...
          if (added.length)
            actions[actions.length - 1] = {
              ...actions[actions.length - 1],
              text: actions[actions.length - 1].text + ' ' + added.join(' '),
            };
          out.push({
            anchor,
            type: 'update-block',
            title: b.title + ': ' + (added[0] ?? 'עדכון'),
            targetDocumentId: s.documentId,
            targetStepKey: s.stepKey,
          ...
          out.push({
            anchor,
            type: 'update-step',
            title: s.stepTitle + ': ' + (added[0] ?? 'שינוי ניסוח'),
            targetDocumentId: s.documentId,
            targetStepKey: s.stepKey,
            targetBlockId: null,
