...
exports.up = (pgm) => {
  pgm.createTable('users', {
    ...
    source: { type: 'text', notNull: true, check: "source in ('entra','paloalto','local')" },
    email: 'text',
    display_name: { type: 'text', notNull: true },
    initials: { type: 'text', notNull: true, default: '' },
  ...
  pgm.createTable('roles', {
    ...
    system: { type: 'boolean', notNull: true, default: false },
    created_at: { type: 'timestamptz', notNull: true, default: pgm.func('now()') },
  });
  ...
  pgm.createTable(
    'role_permissions',
    {
      ...
      permission: { type: 'text', notNull: true, references: 'permissions', onDelete: 'cascade' },
    },
    { constraints: { primaryKey: ['role_id', 'permission'] } },
  );
  pgm.createTable(
    'user_roles',
    {
  ...
  pgm.createTable('groups_map', {
    idp_group_id: { type: 'text', primaryKey: true },
    idp_group_name: { type: 'text', notNull: true },
    role_id: { type: 'uuid', notNull: true, references: 'roles', onDelete: 'cascade' },
  });
  for (const p of PERMISSIONS)
    pgm.sql(`insert into permissions(name, resource) values ('${p}', '${p.split('.')[0]}')`);
  for (const [name, perms] of Object.entries(ROLES)) {
    pgm.sql(`insert into roles(name, system) values ('${name}', true)`);
    for (const p of perms)
      pgm.sql(
        `insert into role_permissions(role_id, permission) select id, '${p}' from roles where name='${name}'`,
      );
  }
};
exports.down = (pgm) => {
  pgm.dropTable('groups_map');
