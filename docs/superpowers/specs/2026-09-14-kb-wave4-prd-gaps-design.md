...
- `documents.doc_type text not null check (doc_type in ('M','R','O','E','S','T','I'))`. Backfill: a legacy `code` prefix wins when present (`M-` → M, `R-` → R, `O-` → O, `E-` → E, `S-` → S, `T-` → T, `I-` → I); otherwise rows with ≥1 phase → `R`, rows with no phases → `I`, kind `retention` → `R`.
...
- TipTap (StarterKit + Table, Image, Link, TextAlign, Underline), `dir="rtl"` default, Hebrew toolbar. Server-side sanitizer allowlist: `h1–h4, p[dir], ul, ol, li, table, thead, tbody, tr, th[colspan|rowspan], td[colspan|rowspan], img[src=/api/v1/assets/*] (data: URIs rejected), a[href http(s)], strong, em, u, s, blockquote, code, pre, br, hr, span[dir], bdi`. Anything else is stripped; the stored HTML is therefore always renderable by WordPress and the docx exporter. (`p[dir]` and the two span attributes were added in the wave-4 fix pass — C-M5: TipTap emits RTL paragraphs and merged cells, both survive to docx, and `colspan`/`rowspan` are constrained to `/^\d{1,2}$/`. `a` is rendered with `rel="noopener noreferrer"`; `target` is stripped.)
